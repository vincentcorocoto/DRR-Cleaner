...
def process_file(filepath):
    ...
    # Re-number S.No on the cleaned output sequentially (1, 2, 3...)
    # AFTER rows have been removed, so the numbers match the retained rows in order.
    # All other columns (Debtor, Date, Time, DPD etc.) remain correctly aligned.
    if _sno_col in cleaned_df.columns:
        cleaned_df[_sno_col] = [str(i) for i in range(1, len(cleaned_df) + 1)]
...
class AutostatScreen(tk.Frame):
    ...
    def _import_thread(self):
        try:
            ...
            # Apply the fixed header names positionally (extra columns, if any,
            # keep whatever name the file itself had for them).
            ...
            df = df.dropna(how="all")               # rows that are entirely NaN

            # Reformat Remark Date to 24-hour "mm/dd/yyyy HH:mm:ss" (no AM/PM).
            if "Remark Date" in df.columns:
                parsed = pd.to_datetime(df["Remark Date"], errors="coerce")
                formatted = parsed.dt.strftime("%m/%d/%Y %H:%M:%S")
                df["Remark Date"] = formatted.where(parsed.notna(), df["Remark Date"])

            df = df.fillna("")
            blank_mask = (df.astype(str).apply(lambda c: c.str.strip()) == "").all(axis=1)
            df = df[~blank_mask].reset_index(drop=True)  # rows that are entirely empty strings
