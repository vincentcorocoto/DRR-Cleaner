...
class CleanerApp(tk.Tk):
    ...
    def _switch_tab(self, tab_id):
        ...
        colors = {"cleaned": ACCENT, "removed": CARD, "remarks": CARD}
        colors[tab_id] = ACCENT2 if tab_id == "remarks" else ACCENT
        ...
        if df is not None and not (hasattr(df, 'empty') and df.empty):
            ...
            self._clear_table()
            self.row_count_label.config(text="No remark changes found." if tab_id == "remarks" else "No rows.")
