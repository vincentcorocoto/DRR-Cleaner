...
def _app_data_dir():
    ...
    - Running as a frozen PyInstaller .exe: %APPDATA%\\CleanDRR on Windows
      (or the user's home folder on other OSes), since the PyInstaller
      onefile temp extraction folder is wiped after every run."""
    if getattr(sys, "frozen", False):
        base = os.environ.get("APPDATA") or os.path.expanduser("~")
        folder = os.path.join(base, "CleanDRR")
...
def process_file(filepath):
    ...
    reason  = reason.where(~remark_exclude_mask.reindex(removed_df.index, fill_value=False),
                            "Remark contains excluded phrase")
...
class DRRCleanerScreen(tk.Frame):
    ...
    def _build_ui(self):
        ...
        self.download_btn = tk.Button(left, text="⬇  Download Output (2 files)",
                                      font=("Segoe UI", 9, "bold"),
    ...
    def _download(self):
        ...
        if not save_path:
            return
        try:
            # ── File 1: Full output (Cleaned, Removed, Remarks, Unique ID) ──
            with open(save_path, "wb") as f:
                f.write(self.output_bytes)

            # ── File 2: Simple output — Cleaned sheet only, same folder ──────
            base, ext = os.path.splitext(save_path)
            simple_path = f"{base}_cleaned_only{ext}"
            simple_buf = io.BytesIO()
            with pd.ExcelWriter(simple_buf, engine="openpyxl") as writer:
                frame = self.cleaned_df if self.cleaned_df is not None else pd.DataFrame()
                frame.to_excel(writer, index=False, sheet_name="Cleaned")
            ...
            with open(simple_path, "wb") as f:
                f.write(simple_buf.read())

            self.status_var.set(f"Saved → {save_path}  +  {os.path.basename(simple_path)}")
            show_toast(self.master,
                       f"Saved 2 files: {os.path.basename(save_path)} & {os.path.basename(simple_path)}",
                       color=SUCCESS)
        except Exception as e:
...
class AutostatScreen(tk.Frame):
    ...
    def _open_masterlist_manager(self):
        ...
        tk.Label(win, text="Any Remark value not found in this list will be highlighted red in the table.",
                 font=("Segoe UI", 8), bg=BG, fg=SUBTEXT, wraplength=440, justify="left"
    ...
    def _download_autostat(self):
        ...
        try:
            df = self.imported_df.copy()
            df.to_excel(save_path, sheet_name="Imported Data", index=False)
            ...
            # Highlight only the Remark cell when its value isn't in the Reference list
            if self._masterlist and "Remark" in df.columns:
                ...
                for row in range(2, ws.max_row + 1):
                    remark_val = str(ws.cell(row=row, column=remark_col_idx).value or "").strip().lower()
                    if remark_val and remark_val not in masterlist_lower:
    ...
    def _build_ui(self):
        ...
        tk.Button(tab_bar, text="⬇  Download", font=("Segoe UI", 9, "bold"),
    ...
    def _import_thread(self):
        try:
            is_csv = self.file_path.lower().endswith(".csv")

            # Row 1 is always a title/junk row — skip it.
            # Row 2 becomes the header; data starts from row 3.
            if is_csv:
                df = pd.read_csv(self.file_path, dtype=str, skiprows=1)
            else:
                df = pd.read_excel(self.file_path, dtype=str, skiprows=1)
            ...
            # Reformat date columns to "MM/DD/YYYY HH:MM:SS" (24-hour, no AM/PM).
            for date_col in ("Remark Date", "Claim Paid Date"):
                if date_col in df.columns:
                    df[date_col] = df[date_col].apply(self._format_remark_date)
    ...
    def _render_page(self):
        ...
        # Compare Status against the Reference list (same masterlist used by Remark)
        ...
        for i, vals in enumerate(disp_df.values.tolist()):
            ...
            # ── Status check against Reference list ───────────────────────────
            if status_idx is not None and masterlist_lower:
                status_val = str(vals[status_idx]).strip()
                if status_val and status_val.lower() not in masterlist_lower:
        ...
        total_pages = max(1, (len(df) + self.PAGE_SIZE - 1) // self.PAGE_SIZE)
        flag_note = f"  ·  🔴 {status_flagged} Status not in Reference" if (status_flagged and masterlist_lower) else ""
    ...
    def _on_tree_click(self, event):
        ...
        # Single-click on a Status cell that is invalid → open suggestion dropdown
        if col_name == "Status":
            masterlist_lower = {m.lower() for m in self._masterlist} if self._masterlist else set()
            if masterlist_lower and val.strip() and val.strip().lower() not in masterlist_lower:
    ...
    def _on_double_click(self, event=None):
        ...
        if event and event.x:
            ...
            try:
                ...
                pass
        if clicked_col == "Remark":
            self._edit_remark_cell(item)
            return
        self._show_row_detail(columns, values, clicked_col)
...
class App(_AppBase):
    def __init__(self):
        super().__init__()
        self.title("Operations Portal  [v2 - SNO+DPD Fixed]")
